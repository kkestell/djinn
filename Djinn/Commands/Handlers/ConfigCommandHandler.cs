...
public class ConfigCommandHandler : ICommandHandler
{
    public Task<int> InvokeAsync(InvocationContext context)
    {
        ...
        Log.Information($"Ffprobe path:            {config.FfprobePath}");
        ...
        Log.Information($"Cover display style      {config.CoverDisplayStyle}");
        Log.Information($"Strip existing metadata: {config.StripExistingMetadata}");
